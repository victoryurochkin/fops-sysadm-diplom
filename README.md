...
В ходе выполнения курсового проекта была успешно спроектирована и реализована отказоустойчивая инфраструктура для веб-приложения victoryur.tech, соответствующая современным требованиям к эксплуатации веб-сервисов.

**В соответствии с поставленными целями и задачами были выполнены следующие этапы:**

- Развёрнута аппаратная платформа на базе гипервизора Proxmox VE, созданы необходимые виртуальные машины, сконфигурирована сетевая инфраструктура с использованием межсетевого экрана OPNSense, обеспечивающего NAT и базовую фильтрацию трафика.
- Установлен и настроен Web Application Firewall (BunkerWeb), осуществляющий защиту прикладного уровня от типовых атак (SQLi, XSS) и выполняющий функции reverse proxy для направления запросов к внутренним серверам в соответствии с доменными именами.
- Сконфигурирован балансировщик нагрузки на базе Nginx, реализующий алгоритм least_conn и обеспечивающий равномерное распределение запросов между двумя бэкенд-серверами, а также автоматическое исключение отказавшего узла из пула.
- Развёрнуты два идентичных бэкенд-сервера с веб-сервером Nginx, содержащих тестовое веб-приложение; на них установлены и настроены агенты мониторинга Zabbix и сбора логов Filebeat.
- Создана система централизованного мониторинга на базе Zabbix 7.0 с хранением метрик в MariaDB, обеспечивающая сбор и визуализацию данных о состоянии всех узлов инфраструктуры.
- Развёрнут стек ELK (Elasticsearch + Kibana) для централизованного хранения и анализа логов; настроена передача логов Nginx с бэкендов с помощью Filebeat, обеспечена возможность построения дашбордов и поиска по событиям.
- Организовано автоматическое резервное копирование всех виртуальных машин средствами Proxmox с ежедневным расписанием и хранением двух последних копий, что гарантирует восстановление после сбоев.
- Разработаны Ansible-плейбуки для полного воспроизведения конфигурации инфраструктуры, демонстрирующие применимость методологии Infrastructure as Code и возможность масштабирования.
- Проведённое интеграционное тестирование подтвердило корректную работу всех компонентов: балансировка запросов осуществляется равномерно, при отказе одного из бэкендов трафик автоматически перенаправляется на работающий узел без прерывания обслуживания; метрики мониторинга и логи централизованно собираются и доступны для анализа. Принятые меры безопасности (сегментация сети, использование WAF, аутентификация по ключам) обеспечивают защиту от несанкционированного доступа и типовых атак.

Таким образом, разработанная инфраструктура полностью отвечает критериям отказоустойчивости, наблюдаемости и безопасности.
Все поставленные задачи выполнены в полном объёме, цель проекта достигнута.
